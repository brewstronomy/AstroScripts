...
npix = 1e4
cmap = cm.Spectral_r
csca = cm.ScalarMappable(cmap=cmap, norm=cols.Normalize(vmin=-500, vmax=500))
cfun = lambda x: cols.to_hex(csca.to_rgba(x))

origin = [0], [0]
qwarg = {'angles': 'xy', 'scale_units': 'xy', 'scale': 1, 'width': 0.005}
ckwarg = {'cmap': cmap, 'vmin': -500, 'vmax': 500}
...
func_VLS = lambda x: (318 + np.array([np.dot(c.xyz.value, [234, -31, 214]) for c in np.array(x)]))*u.km/u.s

v_LS = func_VLS(coords_galacti_unitCart)  # array
v_PA_LS = func_VLS([coord_PA_galacti_unitCart])[0]  # value
v_closeGal_LS = func_VLS([coord_closeGal_galacti_unitCart])[0]  # value

# peculiar (v_GSR - Hubble flow)
H0 = 74.03*u.km/u.s/u.Mpc  # km/s/Mpc; SHoES collab (Riess+2019)
vpec_GSR = vrad_GSR - H0 * coords.distance
vpec_PA_GSR = vrad_PA_GSR- H0 * coord_PA.distance
vpec_closeGal_GSR = vrad_closeGal_GSR- H0 * coord_closeGal.distance
...
# plot
fig, ax = plt.subplots()
ax.scatter(dist, vpec_GSR.value, alpha=0.2, edgecolor='gray', label='', zorder=0)
...
ax.scatter(coord_closeGal.distance.value, vpec_closeGal_GSR.value, s=200, edgecolor='black', linewidths=1,
           color='tab:green', label=name_closeGal, zorder=4)
ax.scatter(coord_PA.distance.value, vpec_PA_GSR.value, s=400, marker='*', edgecolor='black', linewidths=1,
           color='tab:orange', label='Pisces A', zorder=4)
...
akwargs = lambda x: {'edgecolor': 'black', 'facecolor': cfun(vpec_PA_GSR.value), 'linewidth': 1,
...
fig, ((ax11, ax12), (ax21, ax22)) = plt.subplots(2, 2, figsize=(10, 10), sharey=True, sharex=True)
fig.subplots_adjust(wspace=0, hspace=0)
# Y vs Z
gals = ax11.scatter(Z_supergal, Y_supergal, c=vpec_GSR.value, s=100, edgecolor='black', linewidths=0.5, label='nearby galaxies', zorder=1, **ckwarg)
pa = ax11.scatter(Z_PA_supergal, Y_PA_supergal, c=cfun(vpec_PA_GSR.value),
...
# Y vs X
im = ax12.scatter(X_supergal, Y_supergal, c=vpec_GSR.value, s=100, edgecolor='black', linewidths=0.5, label='nearby galaxies', zorder=1, **ckwarg)
ax12.scatter(X_PA_supergal, Y_PA_supergal, c=cfun(vpec_PA_GSR.value), s=300, marker='*', edgecolor='black',
...
cbar.ax.set_xlabel('Peculiar Velocity ($v_{\\rm GSR}$/km s$^{-1}$)')
...
# X vs Z
labax = ax22.twinx()
labax.scatter(X_supergal, Z_supergal, c='none', zorder=0)
labax.set_ylabel('$Z_{\\rm SG}$ (Mpc)', rotation=270, labelpad=10)

ax22.scatter(X_supergal, Z_supergal, c=vpec_GSR.value, s=100, edgecolor='black', linewidths=0.5, label='nearby galaxies', zorder=1, **ckwarg)
ax22.scatter(X_PA_supergal, Z_PA_supergal, c=cfun(vpec_PA_GSR.value), s=300, marker='*', edgecolor='black', label='Pisces A', zorder=3, **ckwarg)
...
vpec_gal_LS = vrad_GSR.value[maskLS] - H0*dist[maskLS]
...
ymin, ymax = ax.get_ylim()
ax.plot([PA_vpec_gal, PA_vpec_gal], [ymin, ymax], color='tab:red', linestyle='dashed', linewidth=2, zorder=1)
ax.annotate('\\textbf{Pisces A}', (0.7*PA_vpec_gal, 1), ha='left', va='center', color='tab:red')
...
vpec_gal_oLS = vrad_GSR.value[maskoLS] - H0*dist[maskoLS]
...
ymin, ymax = ax.get_ylim()
ax.plot([PA_vpec_gal, PA_vpec_gal], [ymin, ymax], color='tab:red', linestyle='dashed', linewidth=2, zorder=1)
ax.annotate('\\textbf{Pisces A}', (1.7*PA_vpec_gal, 0.95*ymax),
...
movie.save(path.join(con.fullpath + 'Tests/', 'PA_3Ddistribution_animation.mp4'), fps=30)
plt.close()
del movie
